...
with st.expander("확보하지 못한 항목 (개인 / 기관 세부 / 기타법인) — 결론: 로그인 없이는 불가능, 시도하지 않음"):
    st.markdown(
        "요청 항목(개인, 기타법인, 기관 세부: 금융투자·보험·투신·사모펀드·은행·기타금융·"
        "연기금등·국가지자체)은 **비로그인 소스가 존재하지 않아 이 앱에서 시도하지 "
        "않습니다.** 아래 3가지 경로를 모두 직접 테스트해 확인했습니다.\n\n"
        "1. **KRX 데이터 API** (`data.krx.co.kr`의 `getJsonData.cmd`): 비로그인 세션으로 "
        "직접 POST 요청 → `HTTP 400 LOGOUT` 거부\n"
        "2. **KRX CSV/엑셀 다운로드** (`GenerateOTP/generate.cmd` → `download.cmd`): 같은 "
        "비로그인 세션으로 다운로드 토큰 발급을 요청 → 역시 `LOGOUT` 거부. 브라우저의 "
        "CSV 다운로드 버튼도 동일한 내부 API를 쓰므로 로그인 없이는 다운로드도 불가합니다.\n"
        "3. **네이버 증권**: 종목 메인 페이지와 수급 페이지(frgn.naver) 원문을 전부 "
        "검색해봐도 '연기금', '금융투자', '사모' 등 세부 구분 문자열 자체가 없습니다 — "
        "이 페이지들은 애초에 외국인/기관(합계)만 제공합니다.\n\n"
        "따라서 이 대시보드는 로그인 없이 얻을 수 있는 **외국인 / 기관(합계)** 두 항목만 "
        "제공합니다. KRX 로그인(KRX_ID/KRX_PW)을 다시 붙이면 세부 항목을 받을 수 있지만, "
        "그건 이전에 로그인 방식을 없앤 이유와 정면으로 배치되어 지금은 적용하지 않았습니다."
    )
